    ...
    <>
      ...
      {!isEmpty && (
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-40 border-t border-maroon/15 bg-cream/85 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-6px_20px_rgba(0,0,0,0.12)] backdrop-blur-md lg:hidden">
            <Link
              href={`/checkout/${restaurantSlug}`}
              className="mx-auto flex w-full max-w-md items-center justify-between rounded-full bg-maroon px-5 py-3 text-sm font-semibold text-cream shadow-md transition-colors hover:bg-maroon-dark"
            >
              <span>
